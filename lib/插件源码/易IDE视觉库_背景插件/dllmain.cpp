...
int CALLBACK IDraw_NotifyCallback(UINT nCode, WPARAM wParam, LPARAM lParam)
{
    ...
    {
        if (wParam < 0 || wParam > 7)   // 不是0-7的不处理重画
            return 0;
    ...
    case INE_MENU_GETICON:
    {
        ...
        if (wParam == 1000) // 设置按钮
        {
            ...
            return (int)iDraw_GetIcon(g_hResource, id, icon->prc, icon->state, icon->pIconType);
        }
        break;
    }
    default:
...
extern "C" _declspec(dllexport) void* WINAPI IDraw_Interface(LPIDRAW_INFO pInfo, IGetEWindow* eWnd, int reserved1, int reserved2)
{
    ...
    //InsertMenuA((HMENU)DLL_GUID, -1, uFlags, 2000, "支持调用 InsertMenuA 来添加");
    //InsertMenuW((HMENU)DLL_GUID, -1, uFlags, 2001, L"支持调用 InsertMenuW 来添加");
    //AppendMenuA((HMENU)DLL_GUID, uFlags, 2002, "支持调用 AppendMenuA 来添加");
    //AppendMenuW((HMENU)DLL_GUID, uFlags, 2003, L"支持调用 AppendMenuW 来添加");

    MENUITEMINFOW item = { 0 };
    item.cbSize = sizeof(item);
    //item.fMask = MIIM_DATA | MIIM_ID | MIIM_STRING | MIIM_TYPE | MIIM_STATE;
    //item.fType = MFT_STRING | 0;
    //item.fState = MFS_DISABLED;
    //item.wID = 2004;
    //item.dwItemData = 123456789;
    //item.dwTypeData = (LPWSTR)L"InsertMenuItemW 添加的被禁止的菜单";
    //InsertMenuItemW((HMENU)DLL_GUID, -1, true, &item);

    //item.fMask = MIIM_DATA | MIIM_ID;
    //item.wID = 7;
    //item.dwItemData = (LPARAM)g_image[7];
    //item.dwTypeData = 0;
    //// 上面把第7个屏蔽了, 这里使用这种方式插入到id是-1的菜单前面, 就是插入到分隔条的前面
    //InsertMenuItemW((HMENU)DLL_GUID, -1, false, &item);
