...
public:
    ...
    virtual LPBYTE _img_save_frombitmap(ID2D1Image* pBitmap, SIZE_T& size, UINT left, UINT top, UINT right, UINT bottom) = 0;

    // 从绘画句柄里获取图片数据, 返回图片数据指针和尺寸, 需要调用 _img_free 释放
    virtual LPBYTE _img_save_formcanvas(const LPCANVAS hCanvas, SIZE_T& size) = 0;
    // 从绘画句柄指定左顶宽高获取图片数据, 返回图片数据指针和尺寸, 需要调用 _img_free 释放
    virtual LPBYTE _img_save_formcanvasex(const LPCANVAS hCanvas, SIZE_T& size, UINT left, UINT top, UINT right, UINT bottom) = 0;
    // 从图片句柄获取图片数据, 返回图片数据指针和尺寸, 需要调用 _img_free 释放
    virtual LPBYTE _img_save(LPEX_IMAGE image, SIZE_T& size) = 0;
    // 从图片句柄指定左顶宽高获取图片数据, 返回图片数据指针和尺寸, 需要调用 _img_free 释放
    virtual LPBYTE _img_save_fromimage(LPEX_IMAGE image, SIZE_T& size, UINT left, UINT top, UINT right, UINT bottom) = 0;
